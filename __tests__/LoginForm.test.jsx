...
describe('login', () => {
  test('successful player login', async () => {
    ...
    fireEvent.change(usernameInputPlayer1, { target: { value: NORMAL_USERNAME } });
    ...
    const usernameInputPlayer2 = await screen.findByTestId('usernameInputPlayer2');
  ...
  test('input errors', async () => {
    ...
    const requiredUsernameError = await screen.findByText(i18n.t('errors.required'));
    expect(requiredUsernameError).toBeInTheDocument();

    // test tooShortUsername error
    fireEvent.change(usernameInputPlayer1, { target: { value: TOO_SHORT_USERNAME } });
    expect(usernameInputPlayer1.value).toBe(TOO_SHORT_USERNAME);

    fireEvent.click(submitUsername);

    const tooShortUsernameError = await screen.findByText(i18n.t('errors.tooShortUsername'));
    expect(tooShortUsernameError).toBeInTheDocument();

    // test tooLongUsername error
    fireEvent.change(usernameInputPlayer1, { target: { value: TOO_LONG_USERNAME } });
    expect(usernameInputPlayer1.value).toBe(TOO_LONG_USERNAME);

    fireEvent.click(submitUsername);

    const tooLongUsernameError = await screen.findByText(i18n.t('errors.tooLongUsername'));
    ...
    renderWithProviders(
      <LoginForm />,
      { preloadedState: { gameData: { ...initialState, players: [NORMAL_USERNAME] } } },
    );
    const usernameInputPlayer2 = await screen.findByTestId('usernameInputPlayer2');
    fireEvent.change(usernameInputPlayer2, { target: { value: NORMAL_USERNAME } });
    ...
    const duplicatedUsernameError = await screen.findByText(i18n.t('errors.duplicatedUsername'));
